...
> = ({ results, className = "" }) => {
  ...
  if (
    results.summary.securityScore >= 80 &&
      ...
      <CardContent>
        <div className="space-y-3">
          {recommendations.map((rec, index) => (
            <div
              key={index}
              className={`flex items-start gap-3 p-3 ${rec.bgColor} rounded-lg`}
            >
              <rec.icon className={`h-5 w-5 ${rec.iconColor} mt-0.5`} />
              <div>
                <p className={`font-medium ${rec.titleColor}`}>{rec.title}</p>
                <p className={`text-sm ${rec.descColor}`}>{rec.description}</p>
              </div>
            </div>
          ))}

          {recommendations.length === 0 && (
            <div className="flex items-start gap-3 rounded-lg bg-muted p-3 dark:bg-teal-900/20">
              ...
              <div>
                ...
                <p className="text-sm text-primary dark:text-primary">
                  Your code appears to be in good shape. Continue following
