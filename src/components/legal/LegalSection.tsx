  ...
  return (
    <section className={`mb-8 sm:mb-12 ${className}`}>
      ...
      <div className="mb-6 flex items-center gap-3 border-b border-border pb-4">
        ...
        <ChevronRight className="ml-auto h-6 w-6 text-muted-foreground" />
      </div>
...
export const LegalSubsection: React.FC<LegalSubsectionProps> = ({
  ...
  return (
    <div className={`mb-6 ${className}`}>
      ...
      <div className="ml-4 space-y-3 text-foreground/80">
        {children}
      </div>
    </div>
  );
      ...
      {items.map((item, index) => (
        <li key={index} className="flex items-start gap-3">
          <div className="mt-2 flex-shrink-0">
            {ordered ? (
              <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-primary text-sm font-bold text-white">
                {index + 1}
              </span>
            ) : (
