...
export const useFileUpload = ({
  ...
  const [dependencyScanner] = useState(
    () => new DependencyVulnerabilityScanner()
  );
  const [currentAnalysisFile, setCurrentAnalysisFile] = useState<string | null>(
    null
  );
  ...
  const startProgressTracking = useCallback((fileSize: number) => {
    ...
    progressInterval.current = setInterval(() => {
      ...
      const progressRatio = Math.min(
        elapsed / estimatedTotalTime.current,
        0.95
      ); // Cap at 95%
      ...
      if (phaseNumber !== lastPhaseNumber) {
        ...
        if (actualPhaseDurations.current.length >= 2) {
          const avgPhaseDuration =
            actualPhaseDurations.current.reduce((a, b) => a + b, 0) /
            actualPhaseDurations.current.length;
          ...
          estimatedTotalTime.current =
            (estimatedTotalTime.current + adaptiveEstimate) / 2;
        }
      }
      ...
      try {
        ...
        try {
          ...
          if (file.name.toLowerCase().endsWith(".zip")) {
            try {
              const zipAnalysis = await zipAnalysisService.analyzeZipFile(
                file as unknown as {
                  name: string;
                  size: number;
                  lastModified: number;
              ...
              logger.warn(
                "ZIP/dependency analysis failed, continuing with core results:",
                error_
              );
            }
          }
          ...
          if (
            ...
            analysisError.message.includes("does not contain any code files")
          ) {
